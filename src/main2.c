...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    ...
    UINTN num_handles = 0;
    status = uefi_call_wrapper(BS->LocateHandleBuffer, 5, ByProtocol, &gEfiBlockIoProtocolGuid, NULL, &num_handles, &block_io_handle);
    ASSERT(!EFI_ERROR(status));

    // Process Block IO Handle
    for (UINTN i = 0; i < num_handles; i++) {
        status = uefi_call_wrapper(BS->HandleProtocol, 3, block_io_handle[i], &gEfiBlockIoProtocolGuid, (VOID **)&block_io);
        if (!EFI_ERROR(status)) {
            ...
            if (!EFI_ERROR(status)) {
                dev_path_str = ConvertDevicePathToText(dev_path, TRUE, TRUE);
                Print(L"Device %s\n:", dev_path_str);
            ...
            if (media && media->BlockSize != 0) {
                EFI_PARTITION_ENTRY *part_entry;
                EFI_GUID part_type_guid = {0};
                CHAR16 *part_type_str;

                for (UINT32 j = 0; j < 128; j++) {
                    status = ReadGptPartitionEntry(block_io, j, &part_entry);
                    if (!EFI_ERROR(status)) {
                        // if ESP
                        if (CompareGuid(&part_entry->PartitionTypeGUID, &gEfiPartTypeSystemPartitionGuid)) {
                            part_type_str = L"EFI SYSTEM PARTITION (ESP)";
                        } else if (CompareGuid(&part_entry->PartitionTypeGUID, &gEfiPartTypeBasicDataGuid)) {
                            part_type_str = L"Basic Data Partition";
                        } else {
                            part_type_str = L"Unknown";
                        }

                        Print(L"Partition %d : %s", j, part_type_str);
                    }
                }
            }
        }
